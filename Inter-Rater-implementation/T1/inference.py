...
def get_concept_ids(metadata, max_concepts=None, logger=None):
    concept_ids = [metadata[i]["concept_id"] for i in range(len(metadata))]
    if max_concepts:
        if logger is not None:
            logger.warning(f"All ranks only processing {max_concepts} concepts")
...
def infer_steering(args, rank, world_size, device, logger, training_args, generate_args, suppress_eval_dir=None):
    ...
    concept_ids = get_concept_ids(metadata, args.max_concepts, logger)
